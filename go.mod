...
// ga-9ae7o (Mayor, 2026-08-07): o vendor agora e o v1.1.0 LIMPO, sem patch local.
//
// O antigo patch gc-aov9u (SetConnMaxIdleTime 20s no pool Dolt de daemon longo,
// para aposentar conexao ociosa antes do reaper de 30s do servidor) foi ABSORVIDO
// upstream: o 1.1.0 traz defaultConnMaxIdleTime = 20 * time.Second em
// internal/storage/dolt/store.go, com o mesmo valor. Conferido antes de largar o
// patch — nao presuma, o comando esta no bead.
//
// POR QUE ISTO IMPORTOU: enquanto o vendor ficou em 1.0.5 e o binario bd em 1.1.0,
// o gc reprovava no gate de compatibilidade (native_store_unavailable) e caia pro
// caminho degradado, disparando UM PROCESSO bd por chamada. Isso empurrava a
// concorrencia de base do Dolt para o joelho medido (9-15 conexoes, ga-7j5vf) e,
// a partir dai, spawn de revisor, envio de mail e criacao de sessao passavam a
// falhar em cascata.
//
// A copia continua vendorizada (nao modulo publicado) porque o replace abaixo e o
// que permite build reprodutivel sem depender da rede. Se um dia voltar a divergir
// do upstream, DOCUMENTE aqui o que mudou — foi a ausencia desse registro que fez
// o patch anterior parecer perdido.
