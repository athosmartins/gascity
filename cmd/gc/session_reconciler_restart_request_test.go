...
func (e *restartRequestTestEnv) reconcile(sessions []beads.Bead) {
	...
	_ = e.reconcileWithPoolDesiredAndDrainOps(sessions, poolDesired, nil)
}

func (e *restartRequestTestEnv) reconcileWithPoolDesiredAndDrainOps(sessions []beads.Bead, poolDesired map[string]int, dops drainOps) int {
...
func TestReconcileSessionBeads_ResetOfLiveSessionIsNotAtomicWithinTick(t *testing.T) {
	...
	poolDemandZero := map[string]int{"worker": 0}
	if got := env.reconcileWithPoolDesiredAndDrainOps([]beads.Bead{session}, poolDemandZero, nil); got != 0 {
		t.Fatalf("tick 1 start attempts = %d, want 0 so alias release stays on a separate tick", got)
	}
	if env.sp.IsRunning(sessionName) {
		t.Fatalf("session %q still running after tick 1 reset stop", sessionName)
	}
	stopped, err := env.store.Get(session.ID)
	if err != nil {
		t.Fatalf("Get(%s): %v", session.ID, err)
	}
	if stopped.Metadata["continuation_reset_pending"] != "true" {
		t.Fatalf("continuation_reset_pending = %q, want true after tick 1", stopped.Metadata["continuation_reset_pending"])
	}
	if stopped.Metadata[sessionpkg.ResetCommittedAtKey] == "" {
		t.Fatal("reset_committed_at = empty, want durable tick-2 wake marker")
	}
	...
	input := buildAwakeInputFromReconciler(
		...
		nil,
		nil,
		[]wakeTarget{{session: &stopped, tp: env.desiredState[sessionName], alive: false}},
		env.sp,
		env.clk.Now(),
	)
	decision := ComputeAwakeSet(input)[sessionName]
	if !decision.ShouldWake || decision.Reason != "reset-pending" {
		t.Fatalf("tick 2 awake decision = %+v, want reset-pending wake with zero pool demand", decision)
	}

	if got := env.reconcileWithPoolDesiredAndDrainOps([]beads.Bead{stopped}, poolDemandZero, nil); got != 1 {
		t.Fatalf("tick 2 start attempts = %d, want reset-pending start", got)
	}
	if !env.sp.IsRunning(sessionName) {
		t.Fatalf("session %q did not wake on tick 2 from reset-pending", sessionName)
	}
}
