...
var listCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if skipBody, _ := cmd.Flags().GetBool("skip-body"); skipBody {
			// ga-ftmci: narrow the projection to drop the three large body
			// columns (design, acceptance_criteria, notes). Output JSON/text
			// simply shows them empty; callers using --skip-body declare they
...
func init() {
	...
	// Body hydration toggle (ga-ftmci). Drops the three large body columns
	// (design, acceptance_criteria, notes) from the projection. Output shows
	// them empty. Use only when the caller does not need body text.
	listCmd.Flags().Bool("skip-body", false,
