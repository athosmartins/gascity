...
// IssueSelectColumnsLite is IssueSelectColumns with the three large LONGTEXT
// body columns (design, acceptance_criteria, notes) replaced by empty-string
// literals. The column count, order, and types remain identical to
// IssueSelectColumns, so ScanIssueFrom scans either projection without change;
// the body fields simply come back empty. Used by the SkipBody escape hatch
// (ga-ftmci) so full-table scans that never need body text — the gc supervisor
// reconcile — stop streaming kilobytes per row from the DB engine every cycle.
const IssueSelectColumnsLite = `id, content_hash, title, description, '' AS design, '' AS acceptance_criteria, '' AS notes,
	       status, priority, issue_type, assignee, estimated_minutes,
	       created_at, created_by, owner, updated_at, started_at, closed_at, external_ref, spec_id,
	       compaction_level, compacted_at, compacted_at_commit, original_size, source_repo, close_reason,
	       sender, ephemeral, no_history, wisp_type, pinned, is_template,
