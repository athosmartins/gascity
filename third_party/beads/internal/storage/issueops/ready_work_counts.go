...
func GetReadyWorkWithCountsInTx(ctx context.Context, tx *sql.Tx, filter types.WorkFilter) ([]*types.IssueWithCounts, error) {
	...
	out, err := runSearchQueryInTx(ctx, tx, IssuesFilterTables, issuePreds.whereSQL, issuePreds.orderBySQL, issuePreds.limitSQL, issuePreds.args, wispDepsExist, false, false)
	if err != nil {
		return nil, err
	}
	...
	wisps, err := runSearchQueryInTx(ctx, tx, WispsFilterTables, wispPreds.whereSQL, wispPreds.orderBySQL, wispPreds.limitSQL, wispPreds.args, true, false, false)
	if err != nil {
...
func prefixIssueColumns(cols string) string {
	raw := strings.ReplaceAll(cols, "\n", " ")
	raw = strings.ReplaceAll(raw, "\t", " ")
	parts := strings.Split(raw, ",")
	for i, p := range parts {
		p = strings.TrimSpace(p)
		// ga-ftmci: the lite projection already emits empty-string body
		// literals as "'' AS design" / "'' AS acceptance_criteria" /
		// "'' AS notes". Those are full select expressions, not bare column
		// names, so prefixing them with the "i." table alias would yield the
		// invalid "i.'' AS design". Leave any expression containing a space
		// (i.e. a literal or AS alias) untouched and only qualify bare columns.
...
// readyWorkIssueColumns is the full-body, "i."-qualified projection used by
// the search-with-counts list query (the dominant bd list path).
var readyWorkIssueColumns = prefixIssueColumns(IssueSelectColumns)

// readyWorkIssueColumnsLite mirrors readyWorkIssueColumns but drops the three
// large LONGTEXT body columns (design, acceptance_criteria, notes) via the lite
// projection. ga-ftmci: the gc supervisor reconcile lists every row but never
// reads body text; this lets SkipBody narrow the projection on the list path,
// not only on the secondary search.go path.
var readyWorkIssueColumnsLite = prefixIssueColumns(IssueSelectColumnsLite)

// readyWorkColumnsFor selects the projection honoring the SkipBody flag.
func readyWorkColumnsFor(skipBody bool) string {
	if skipBody {
