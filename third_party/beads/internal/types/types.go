...
type IssueFilter struct {
	...
	// SkipBody suppresses hydration of the three large LONGTEXT body columns
	// (design, acceptance_criteria, notes) made LONGTEXT by migration 0049.
	// When true the projection NULL-fills them, so Issue.Design,
	// Issue.AcceptanceCriteria and Issue.Notes are returned empty. Callers
	// that only need scheduling/change-detection fields (title, status,
	// description, priority, assignee, labels, deps, metadata) — notably the
	// gc supervisor's cache reconcile (ga-ftmci) — opt in to avoid streaming
	// kilobytes of body text per row on every full-table scan. WARNING: do
